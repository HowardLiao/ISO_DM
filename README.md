...
- **二階 (Tier 2) 程序書／管理辦法 (Procedure)**：回答「**誰做、何時做、如何運作？**」，跨部門流程控制點、RACI 邊界、SLA、交接條件與退件權。由各流程主責 Owner 起草，財務（玉霜）、技術（翊庭）、總務（芮郗）共同覆核。
- **三階 (Tier 3) 標準作業程序與指導書 (SOP / WI / Runbook)**：回答「**實際上怎麼做？**」，第一線人員具體步驟、指令語法、畫面截圖、防呆驗證與 Rollback 回滾。由技術文管（翊庭）與執行主管核准。
